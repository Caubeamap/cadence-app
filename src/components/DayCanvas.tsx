...
export function DayCanvas({ blocks, tasks, nowMin, isToday, dayStart, dayEnd, onDone, onSkip }: Props) {
  ...
  const canvasHeight = (endMin - startMin) * PX_PER_MIN + space.xl;
  ...
  useEffect(() => {
    if (isToday && nowMin > startMin) {
      scrollRef.current?.scrollTo({ y: Math.max(0, (nowMin - startMin) * PX_PER_MIN - 120), animated: false });
    }
        ...
        {hours.map((h) => {
          ...
          return (
            <View key={h} style={[styles.hourRow, { top }]} pointerEvents="none">
              ...
              <View style={[styles.gridline, { backgroundColor: c.hairline }]} />
            </View>
          );
        })}
